...
class StudySettingsControllerTest extends AbstractContainerBaseTest {
    ...
    @WithUserDetails(value = "user", setupBefore = TestExecutionEvent.TEST_EXECUTION)
    @DisplayName("스터디 태그 설정 - GET 성공")
    @Test
    void TagSettingsForm_permissions() throws Exception {
        Account account = accountRepository.findByUsername("user");
        Study study = studyFactory.createStudy("study", account);
    ...
    @WithUserDetails(value = "user", setupBefore = TestExecutionEvent.TEST_EXECUTION)
    @DisplayName("스터디 태그 설정 - GET 실패 (접근 권한 없음)")
    @Test
    ...
    void addTags_success() throws Exception {
        ...
        Tag tag = tagRepository.findByTitle("spring boot").orElseThrow();
        assertTrue(study.getTags().contains(tag));
    }

    @WithUserDetails(value = "user", setupBefore = TestExecutionEvent.TEST_EXECUTION)
    @DisplayName("스터디 태그 추가 - POST 실패 (접근 권한 없음)")
    @Test
    ...
    @WithUserDetails(value = "user", setupBefore = TestExecutionEvent.TEST_EXECUTION)
    @DisplayName("스터디 태크 제거 - POST 실패 (접근 권한 없음)")
    @Test
    ...
    @WithUserDetails(value = "user", setupBefore = TestExecutionEvent.TEST_EXECUTION)
    @DisplayName("스터디 지역 설정 - GET 실패 (접근 권한 없음)")
    @Test
    ...
    @WithUserDetails(value = "user", setupBefore = TestExecutionEvent.TEST_EXECUTION)
    @DisplayName("스터디 지역 추가 - POST 성공")
    @Test
    void addLocations_success() throws Exception {
        Account account = accountRepository.findByUsername("user");
        Study study = studyFactory.createStudy("path", account);
        ...
        assertTrue(study.getLocations().contains(location));
    }
    ...
    void addLocations_fail() throws Exception {
        ...
        Location location = locationRepository.findByCityAndProvince(testLocation.getCity(), testLocation.getProvince());
        assertFalse(study.getLocations().contains(location));
    }
    ...
    @WithUserDetails(value = "user", setupBefore = TestExecutionEvent.TEST_EXECUTION)
    @DisplayName("스터디 지역 제거 - POST 실패 (접근 권한 없음)")
    @Test
    void removeLocations_fail() throws Exception {
        Account account = accountFactory.createAccount("manager");
        Study study = studyFactory.createStudy("path", account);
    ...
    @WithUserDetails(value = "user", setupBefore = TestExecutionEvent.TEST_EXECUTION)
    @DisplayName("스터디 팀원 모집 - POST 실패 (팀원 모집을 1시간 이내 설정한 경우 변경 불가능)")
    @Test
    ...
    @WithUserDetails(value = "user", setupBefore = TestExecutionEvent.TEST_EXECUTION)
    @DisplayName("스터디 팀원 모집 중단 - POST 성공")
    @Test
    ...
    @WithUserDetails(value = "user", setupBefore = TestExecutionEvent.TEST_EXECUTION)
    @DisplayName("스터디 팀원 모집 중단 - POST 실패 (팀원 모집을 1시간 이내 설정한 경우 변경 불가능)")
    @Test
    void stopRecruit_fail() throws Exception {
        Account account = accountRepository.findByUsername("user");
        Study study = studyFactory.createStudy("path", account);
        study.setPublished(true);
        study.setRecruiting(true);
    ...
    void updateStudyPath_fail() throws Exception {
        ...
        studyFactory.createStudy("test-path", account);
    ...
    void updateStudyTitle_fail() throws Exception {
        ...
        Study testStudy = studyFactory.createStudy("test-path", account);
        testStudy.setTitle("test-title");
